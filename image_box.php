...
$image_page = (int) Request::req('image_page');
$image_list = array();
$image_list_time = array();
$image_size = '';
...
if ($exec == "upload") {
    ...
    if ($referer_hdr === '' || stripos($referer_hdr, Request::header('Host')) === false) Error("정상적으로 업로드를 하여 주시기 바랍니다.", "window.close");
    if (stripos($referer_hdr, "image_box.php") === false) Error("정상적으로 업로드를 하여 주시기 바랍니다.", "window.close");
    if (Request::method() == 'GET' ) Error("정상적으로 업로드를 하여 주시기 바랍니다", "window.close");

    foreach (Request::files('upload') as $_uf) {
        $u_tmp = $_uf->tmpName();
        $u_name = $_uf->name();

        if ($u_name) {
            $filesize = filesize($u_tmp);

            // 업로드 용량 체크
            if ($maxDirSize < $filesize + $dirSize) Error("이미지 창고 사용 용량을 초과하였습니다.");

            if ($filesize) {
                if (!$_uf->isValid()) Error('정상적인 방법으로 업로드 해주세요', 'window.close');

                if (preg_match('/\.gif$/', $u_name)) $ext = 'gif';
				elseif (preg_match('/\.jpe?g$/i', $u_name)) $ext = 'jpg';
				else Error("이미지는 gif 또는 jpg 파일을 올려주세요");

				$u_name = bin2hex(random_bytes(16)) . '.' . $ext;

                $size = getimagesize($u_tmp);
                if ($size === false || !$size[2]) Error("이미지 파일을 올려주시기 바랍니다");
                if (!@$_uf->moveTo($path . "/" . $u_name)) Error("이미지 업로드가 제대로 되지 않았습니다");
            }
    ...
    Response::redirect(Request::scriptName() . "?id=$id&image_page=$image_page");
    exit();
}

// 삭제 명령 실행시
if ($exec == "delete" && strlen($no) && $id) {
    if (!z_unlink($path . "/" . ($image_list[(int)$no] ?? ''))) exit("에러");
    Response::redirect(Request::scriptName() . "?id=$id&image_page=$image_page");
    exit();
}
...
<form method=post action="<?= Request::scriptName() ?>" ENCTYPE="multipart/form-data" name=imageList>
<input type=hidden name=exec value="upload">
<input type=hidden name=page value="<?= e($image_page) ?>">
...
<input type=hidden name=i_align value="">
	...
	<tr>
		<td align=left>&nbsp;<font color=444444 style=font-family:tahoma;font-size:7pt><b>Total : <?= e($total) ?> ( <?= e(getfilesize($dirSize)) ?> / <?= e(getfilesize($maxDirSize)) ?>)</td>
		<td align=right><font color=444444 style=font-family:tahoma;font-size:7pt><b><?= e($image_page) ?>/<?= e($total_page) ?> Pages</td>
	</tr>
	</table>
	<br>
...
for ($i = $startNum; $i < $endNum; $i++) {
    ...
    if ($_width) $image_size = " width=" . e($_width) . " ";
    elseif ($_height) $image_size = " height=" . e($_height) . " ";
		...
		<td align=center valign=top height=75>
			...
			<tr>
				<td bgcolor=white align=center >
					<a href="javascript:void(imagecheck('<?= e_js($image_list[$i]) ?>','<?= e_js($size[0]) ?>','<?= e_js($size[1]) ?>'))"><img src="<?= e_js($path) ?>/<?= e_js($image_list[$i]) ?>" border=0 <?= $image_size ?>></a>
				</td>
			</tr>
			<tr>
				<td bgcolor=eeeeee height=20 align=center>
					<img src=images/t.gif border=0 height=2><br>
					<a href="javascript:void(window.open('<?= e_js($path) ?>/<?= e_js($image_list[$i]) ?>','imageBoxViewer','width=<?= e_js($size[0] + 20) ?>,height=<?= e_js($size[1] + 40) ?>,toolbars=no'))"><font color=555555 style=font-size:7pt;font-family:verdana>[<b>view</b>]</font></a>
					<a href=<?= Request::scriptName() ?>?id=<?= e($id) ?>&exec=delete&no=<?= $i ?>&image_page=<?= e($image_page) ?> onclick="return confirm('삭제하시겠습니까?')"><font color=555555 style=font-size:7pt;font-family:verdana>[<b>del</b>]</font></a>
...
?>
	...
	<tr>
		<td align=center height=40>
			<a href=<?= Request::scriptName() ?>?id=<?= e($id) ?>&image_page=1>[First]</a><?php
...
for ($i = $startPageNum; $i <= $endPageNum; $i++) {
    ...
    else echo "<a href=" . Request::scriptName() . "?id=" . e($id) . "&image_page=$i>[$i]</a>";
}
?><a href=<?= Request::scriptName() ?>?id=<?= e($id) ?>&image_page=<?= e($total_page) ?>>[Last]</a>
		</td>
	</tr>
	</table>
</form>
