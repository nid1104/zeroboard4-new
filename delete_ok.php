...
{
    if (!isblank($avoid_ip[$i]) && stripos(Request::clientIp(), $avoid_ip[$i]) !== false && !$is_admin)
        Error(" Access Denied ");
}
...
$s_data += array('no' => '', 'child' => '', 'ismember' => '', 'password' => '', 'file_name1' => '', 'file_name2' => '', 'division' => '', 'depth' => 0, 'prev_no' => '', 'next_no' => '', 'father' => '', 'category' => '');
...
{
    $connect->exec("delete from {$t_board}_{$id} where no=?", [$no]); // 글삭제
    ...
    if ($s_data['depth'] == 0)
    {
        if ($s_data['prev_no']) $connect->exec("update {$t_board}_{$id} set next_no=? where next_no=?", [$s_data['next_no'], $s_data['no']]); // 이전글이 있으면 빈자리 메꿈;;;
        if ($s_data['next_no']) $connect->exec("update {$t_board}_{$id} set prev_no=? where prev_no=?", [$s_data['prev_no'], $s_data['no']]); // 다음글이 있으면 빈자리 메꿈;;;
    }
    else {
        $temp = $connect->value("select count(*) from {$t_board}_{$id} where father=?", [$s_data['father']]);
        if (!$temp) $connect->exec("update {$t_board}_{$id} set child='0' where no=?", [$s_data['father']]); // 원본글이 있으면 원본글의 자식글을 없앰;;;
    }

    // 간단한 답글 삭제
    $connect->exec("delete from {$t_comment}_{$id} where parent=?", [$s_data['no']]);
    ...
    // 회원일 경우 해당 해원의 점수 주기
    if ($member['no'] == $s_data['ismember']) $connect->exec("update $member_table set point1=point1-1 where no=?", [$member['no']]);
}

$query_time = getmicrotime();

Response::redirect("zboard.php?id=$id&page=$page&page_num=$page_num&select_arrange=$select_arrange&desc=$des&sn=$sn&ss=$ss&sc=$sc&keyword=$keyword&sn1=$sn1&divpage=$divpage");
?>
