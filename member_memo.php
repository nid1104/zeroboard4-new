...
// 새쪽지 왔습니다;; 알람 없애기
$connect->exec("update $member_table set new_memo='0' where no=?", [$member['no']]);

// 지정 넘은 글 삭제;;
$connect->exec("delete from $get_memo_table where member_no=? and (" . time() . " - reg_date) >= " . (int)$_zbDefaultSetup['memo_limit_time'], [$member['no']]);

if (in_array($exec, ['del', 'del_all'], true)) check_csrf(true);

// 선택된 메모 삭제;;;
if ($exec == "del_all") {
    for ($i = 0; $i < count($del); $i++) {
        $connect->exec("delete from $get_memo_table where no=? and member_no=?", [$del[$i], $member['no']]);
    }
    Response::redirect(Request::scriptName() . "?page=$page");
}

// 메모삭제
if ($exec == "del") {
    $connect->exec("delete from $get_memo_table where no=? and member_no=?", [$no, $member['no']]);
    Response::redirect(Request::scriptName() . "?page=$page");
}

// 선택된 메모가 있을시 데이타 뽑아오기;;
if ($no) {
    $now_data = $connect->row("select * from $get_memo_table where no=? and member_no=?", [$no, $member['no']]);
    $now_data += array('name' => '', 'user_id' => '');
    if (($now_data['readed'] ?? '') == 1) {
        $connect->exec("update $get_memo_table set readed='0' where no=? and member_no=?", [$no, $member['no']]);
        $check = $connect->value("select count(*) from $get_memo_table where readed='1' and member_no=?", [$member['no']]);
        $connect->exec("update $send_memo_table set readed='0' where reg_date=? and member_to=?", [$now_data['reg_date'], $member['no']]);
        if (!$check) $connect->exec("update $member_table set new_memo='0' where no=?", [$member['no']]);
    }
}

// 읽지 않은 쪽지의 개수 구하기
$new_total = (int)$connect->value("select count(*) from $get_memo_table where readed='1' and member_no=?", [$member['no']]);

// 전체 쪽지의 개수
$total = (int)$connect->value("select count(*) from $get_memo_table  where member_no=?", [$member['no']]);
...
// 데이타 뽑아오는 부분...
$result = $connect->all("select a.no as no, a.subject as subject, a.reg_date as reg_date, a.readed as readed, b.name as name, b.user_id as user_id, a.member_from as member_from from $get_memo_table a ,$member_table b where a.member_no=? and a.member_from=b.no  order by a.no desc limit ?,?", [$member['no'], (int)$start_num, (int)$page_num]);
...
if ($page > $show_page_num) {
    ...
    $print_page = "<a href=" . Requst::scriptName() . "?id=" . e($id) . "&page=" . e($prev_page) . "&select_arrange=" . e($select_arrange) . "&desc=" . e($desc) . "&category=" . e($category) . "&sn=" . e($sn) . "&ss=" . e($ss) . "&sc=" . e($sc) . "&keyword=" . e($keyword) . "&page_num=" . e($page_num) . ">[Prev]</a> ";
    $print_page .= "<a href=" . Requst::scriptName() . "?id=" . e($id) . "&page=1&select_arrange=" . e($select_arrange) . "&desc=" . e($desc) . "&category=" . e($category) . "&sn=" . e($sn) . "&ss=" . e($ss) . "&sc=" . e($sc) . "&keyword=" . e($keyword) . "&page_num=" . e($page_num) . ">[1]</a>..";
}

while ($i + $start_page <= $total_page && $i <= $show_page_num) {
    $move_page = $i + $start_page;
    if ($page == $move_page) $print_page .= " <b>$move_page</b> ";
    else $print_page .= "<a href=" . Requst::scriptName() . "?id=" . e($id) . "&page=" . e($move_page) . "&select_arrange=" . e($select_arrange) . "&desc=" . e($desc) . "&category=" . e($category) . "&sn=" . e($sn) . "&ss=" . e($ss) . "&sc=" . e($sc) . "&keyword=" . e($keyword) . "&page_num=" . e($page_num) . ">[" . e($move_page) . "]</a>";
    $i++;
}

if ($total_page > $move_page) {
    $next_page = $move_page + 1;
    $print_page .= "..<a href=" . Requst::scriptName() . "?id=" . e($id) . "&page=" . e($total_page) . "&select_arrange=" . e($select_arrange) . "&desc=" . e($desc) . "&category=" . e($category) . "&sn=" . e($sn) . "&ss=" . e($ss) . "&sc=" . e($sc) . "&keyword=" . e($keyword) . "&page_num=" . e($page_num) . ">[" . e($total_page) . "]</a>";
    $print_page .= " <a href=" . Requst::scriptName() . "?id=" . e($id) . "&page=" . e($next_page) . "&select_arrange=" . e($select_arrange) . "&desc=" . e($desc) . "&category=" . e($category) . "&sn=" . e($sn) . "&ss=" . e($ss) . "&sc=" . e($sc) . "&keyword=" . e($keyword) . "&page_num=" . e($page_num) . ">[Next]</a>";
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr>
    ...
    <td align=right><img src="images/t.gif" width="10" height="10"><font color="#666666">전체 :
      <b><?= e($total) ?></b> , 새 쪽지 : <b><?= e($new_total) ?></b></font>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  </tr>
</table>

<!-- 선택된 메모가 있을때;; -->
<?php
	if (!empty($now_data['no'])) {

	    $temp_name = get_private_icon($now_data['member_from'], "2");
	    if ($temp_name) $now_data['name'] = "<img src='" . e($temp_name) . "' border=0 align=absmiddle>";
	    $temp_name = get_private_icon($now_data['member_from'], "1");
	    if ($temp_name) $now_data['name'] = "<img src='" . e($temp_name) . "' border=0 align=absmiddle>&nbsp;" . e($now_data['name']);
	    ?>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="3">
        <tr> 
          ...
          <td><img src="images/t.gif" width="10" height="3"><br>
            <a href=javascript:void(window.open('view_info.php?member_no=<?= e($now_data['member_from']) ?>','view_info','width=400,height=500,toolbar=no,scrollbars=yes'))><?= $now_data['name']?></a> <font style=font-size:8pt;>(<b>ID</b> : <?= e($now_data['user_id']) ?>)</td>
        </tr>
        <tr> 
        ...
        <tr> 
          ...
          <td style='word-break:break-all;'><img src="images/t.gif" width="10" height="3"><br>
            <?= autolink(nl2br(e($now_data['memo']))) ?><br>
            <br>
          </td>
        </tr>
        <tr>
          <td align="right" valign="top">&nbsp;</td>
          <td><a href=javascript:void(window.open('view_info.php?member_no=<?= e($now_data['member_from']) ?>','view_info','width=400,height=500,toolbar=no,scrollbars=yes'))><img src="images/memo_reply.gif" width="28" height="15" border=0></a> <a href=<?= Request::scriptName() ?>?exec=del&no=<?= e($no) ?>&page=<?= e($page) ?>&zb_csrf_token=<?= Session::csrfToken() ?> onclick="return confirm('삭제하시겠습니까?')"><img src="images/memo_delete2.gif" width="31" height="15" border=0></a> <a href=<?= Request::scriptName() ?>><img src="images/memo_list.gif" width="18" height="15" border=0></a> </td>
        </tr>
      </table>
    </td>
    ...
    ?>
        ...
        <tr onMouseOver=this.style.backgroundColor="#FFF5F5" onMouseOut=this.style.backgroundColor=""> 
          <td width="20" align="center" height="23"> 
            <input type=checkbox name=del[] value="<?= e($data['no']) ?>">
          </td>
          <td width="20" align="center"><?= $readed ?></td>
          <td style='word-break:break-all;' style=cursor:hand; onclick=location.href="<?= Request::scriptName() . "?exec=view&no=" . e_js($data['no']) . "&page=" . e_js($page) ?>"><img src="images/t.gif" width="10" height="3"><br>
            <a href=<?= Request::scriptName() . "?exec=view&no=" . e($data['no']) . "&page=" . e($page) ?>><?= e($data['subject']) ?></a></td>
          <td width="80" align="center"><img src="images/t.gif" width="10" height="3"><br>
            <a href=javascript:void(window.open('view_info.php?member_no=<?= e($data['member_from']) ?>','view_info','width=400,height=510,toolbar=no,scrollbars=yes'))><?= e($data['name']) ?></a><br><font style=font-size:8pt;color:999999>(<?= e($data['user_id']) ?>)</td>
          <td width="60" align="center"><font style=font-family:Tahoma;font-size:8pt;><span title='<?= e($reg_date) ?>'><?= date("m/d", $data['reg_date']) ?></span></font></td>
        </tr>
<?php
     		$loop_number--;
}
