...
if (isset($_list_check_included) && $_list_check_included) return;
...
function list_check(&$data, $view_check = 0) {
    ...
    if ($keyword) {
        ...
        if ($sn == "on") $name = preg_replace($keyword_pattern, "<span style='color:#FF001E;background-color:#FFF000;'>" . e($keyword) . "</span>", $name);
        if ($ss == "on") $subject = preg_replace($keyword_pattern, "<span color='FF001E' style='color:#FF001E;background-color:#FFF000;'>" . e($keyword) . "</span>", $subject);
    }
    ...
    if (!$setup['only_board']) {
        ...
        if ($data['ismember']) {
            ...
            $data['memo'] = preg_replace($imageBoxPattern, "<img src='icon/member_image_box/$data[ismember]/\\1.\\2' align='\\3' width='\\4' height='\\5' vspace='\\6' hspace='\\7' border='\\8'>", $data['memo']);
        } else {
        }
        ...
        if ($sc == "on" && $keyword) {
            $keyword_pattern = "/" . str_replace("\0", "\\0", preg_quote($keyword, "/")) . "/i";
            $memo = preg_replace($keyword_pattern, "<span style='color:#FF001E;background-color:#FFF000;'>" . e($keyword) . "</span>", $memo);
        }
        ...
        // 아이피
        if ($is_admin) $ip = "IP Address : " . e($data['ip']) . "&nbsp;";
        ...
        $file_name1 = e($data['s_file_name1']);
        $file_name2 = e($data['s_file_name2']);
        ...
        if (preg_match("/\.jpg$/i", $file_name1) || preg_match("/\.gif$/i", $file_name1) || preg_match("/\.png$/i", $file_name1)) $upload_image1 = "<img src=\"" . e($data['file_name1']) . "\" border=0 name=zb_target_resize style=\"cursor:hand\" onclick=window.open(this.src)><br>";
        if (preg_match("/\.jpg$/i", $file_name2) || preg_match("/\.gif$/i", $file_name2) || preg_match("/\.png$/i", $file_name2)) $upload_image2 = "<img src=\"" . e($data['file_name2']) . "\" border=0 name=zb_target_resize style=\"cursor:hand\" onclick=window.open(this.src)><br>";
    }

    // 카테고리의 이름을 구함
    if ($data['category'] && $setup['use_category']) $category_name = e($category_data[$data['category']] ?? '');
    else $category_name = "&nbsp;";
    ...
    if ($prev_no == $data['no']) $number = "<img src=\"" . e($dir) . "/arrow.gif\" border=0 align=absmiddle>"; elseif ($number != "&nbsp;") $number = $loop_number;
