...
$data += array('name' => '', 'no' => '', 'user_id' => '', 'group_no' => '', 'openinfo' => '');

$data['name'] = e($data['name']);

$temp_name = get_private_icon($data['no'], "2");
if ($temp_name) $data['name'] = "<img src='" . e($temp_name) . "' border=0 align=absmiddle>";
$temp_name = get_private_icon($data['no'], "1");
if ($temp_name) $data['name'] = "<img src='" . e($temp_name) . "' border=0 align=absmiddle>&nbsp;" . $data['name'];
$data['name'] = "&nbsp;" . $data['name'] . "&nbsp;";
...
<?php
	    	if ($data['openinfo'] || $member['is_admin'] == 1) {
	    	    ?>
	<a href="view_info2.php?member_no=<?= e($member_no) ?>"><img src=images/vi_B_userinfo.gif border=0></a>
<?php
	    	} else {
	    	    ?>
...
<tr>
  ...
  <td valign=bottom>&nbsp;<font color=brown><b><?= e($data['user_id']) ?> (<?= $data['name'] ?>)</td>
</tr>
<?php
	} else {
	    ?>
