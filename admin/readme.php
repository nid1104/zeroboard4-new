...
<a name=#help1><b>1. 기본 설정</b></a>
    ...
    * E-mail, url, sitename은 꼭 확인하세요
    <font color=red>- 관리자 E-mail : <?= e($_zbDefaultSetup['email']) ?>

    - 사이트 url : <?= e($_zbDefaultSetup['url']) ?>

    - 사이트 이름 : <?= e($_zbDefaultSetup['sitename']) ?></font>

    - 세션 경로 : <?= e($_zbDefaultSetup['session_path']) ?>

    - 게시물 조회 로그 저장 크기 : <?= e($_zbDefaultSetup['session_view_size']) ?>

    - 게시판 추천 로그 저장 크기 : <?= e($_zbDefaultSetup['session_vote_size']) ?>

    - 로그인 유효 시간 : <?= e($_zbDefaultSetup['login_time']) ?>

    - 현재 접속자 검사 : <?= e($_zbDefaultSetup['nowconnect_enable']) ?>

    - 현재 접속자 갱신시간 : <?= e($_zbDefaultSetup['nowconnect_refresh_time']) ?>

    - 현재 접속자 검사시간 : <?= e($_zbDefaultSetup['nowconnect_time']) ?>

    - 한글 아이디 사용 : <?= e($_zbDefaultSetup['enable_hangul_id']) ?>

    - E-Mail 유효 검사 : <?= e($_zbDefaultSetup['check_email']) ?>
