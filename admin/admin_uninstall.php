...
$uninstall = Request::post('uninstall');
$u_hostname = Request::post('u_hostname');
$u_userid = Request::post('u_userid');
$u_password = Request::post('u_password');
$u_dbname = Request::post('u_dbname');
...
if ($exec == "uninstall" && $uninstall == "ok") {
    ...
    $connect = new DB($u_hostname, $u_userid, $u_password, $u_dbname);
    $connect->raw()->query("SET sql_mode = ''");

    $u_dbname = $connect->escapeIdentifier($u_dbname);

    $result = $connect->all("SHOW TABLE STATUS FROM $u_dbname like 'zetyx%'");
    foreach ($result as $data) {
        $data['Name'] = $connect->escapeIdentifier($data['Name']);
        $connect->exec("DROP TABLE {$data['Name']}");
    }

    zRmDir(__DIR__ . '/../data');
    zRmDir(__DIR__ . '/../icon');
    z_unlink(__DIR__ . '/../config.php');
