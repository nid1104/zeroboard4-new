...
<?php
// DB 내의 파일 목록을 구함
$result = $connect->all("select name from $admin_table order by name desc");
$dblist = array();
...
foreach ($result as $bbs) {
    ...
    echo ".";
    flush();
    $nfiles1 = $connect->all("select file_name1 from {$t_board}_{$id} where file_name1 !=''");
    $nfiles2 = $connect->all("select file_name2 from {$t_board}_{$id} where file_name2 !=''");
...
function getFileList($path) {
    ...
    while (($entry = $directory->read()) !== false) {
        if ($entry != "." && $entry != "..") {
            if (is_dir($path . "/" . $entry) && !preg_match('/__zbSessionTMP/i', $path . "/" . $entry)) {
                getFileList($path . "/" . $entry);
            } else {
                if ( !preg_match('/now_connect\.php$/i', $path . "/" . $entry) && !preg_match('/now_member_connect\.php$/', $path . "/" . $entry) && !preg_match('/__zbSessionTMP/', $path . "/" . $entry) ) {
...
?>
	...
	<b>DB에 등록된 파일의 개수 :</b> <?= number_format($totaldblist) ?>


	<b>전체 검색된 파일의 개수 :</b> <?= number_format($totallist) ?>


	<b>쓰레기 파일 개수 :</b> <?= number_format($totaldifflist) ?>
