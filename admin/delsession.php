...
if ($exec == "delete") {
    ...
    while (($entry = $directory->read()) !== false) {
        if ($entry != "." && $entry != "..") {
            if (stripos($entry, Session::id()) === false && stripos($entry, Request::cookie('ZBSESSIONID')) === false) {
                z_unlink($path . "/" . $entry);
                $i++;
                if ($i % 100 == 0) print (".");
                flush();
...
<?php flush(); ?>
	...
	<div align=center>
	<form name=sdc action=<?= Request::scriptName() ?> method=post>
	...
	<tr bgcolor=white>
		<td align=center>개수</td><td><input type=input name=num value="" size=30 style=border:0;height:18px></td>
	</tr>
	<tr bgcolor=white>
...
while (($entry = $directory->read()) !== false) {
    if ($entry != "." && $entry != "..") {
        ...
        if ($i % 100 == 0) {
            ...
            print "document.sdc.size.value='" . e(getfilesize($totalsize)) . "';\n";
        }
        flush();
    }
}
