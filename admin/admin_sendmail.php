...
$total_member = $connect->value("SELECT COUNT(*) FROM {$member_table} WHERE group_no = ? {$s_que}", array_merge([$group_no], $s_params));

if ($total_member == 0) Error("지정된 그룹에 회원이 존재하지 않습니다");
?>
...
<script>
 function check_submit()
 {
  ...
  if(confirm("<?= e($total_member) ?>명의 회원에게 메일을 보내겠습니까?\n\n메일양식을 제대로 선택하였는지를 확인하세요"))
  {
    history.back();
    return true;
  }
...
<td colspan=2><b><?= number_format($total_member); ?> 명의 멤버에게 메일링 보내기</td>
</tr>
...
<tr>
  ...
  <td>&nbsp; <select name=sendnum><option value=1>1</option><option value=10>10</option><option value=100 selected>100</option><option value=200>200</option><option value=300>300</option><option value=400>400</option><option valut=500>500</option></select> 명 단위로 잘라서 메일을 보냅니다</td>
</tr>
