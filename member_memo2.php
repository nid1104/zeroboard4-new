...
// 지정 시간이 넘은 글 삭제;;
$connect->exec("delete from $send_memo_table where member_no=? and (" . time() . " - reg_date) >= " . (int)$_zbDefaultSetup['memo_limit_time'], [$member['no']]);
...
if ($exec == "del_all") {
    for ($i = 0; $i < count($del); $i++) {
        $connect->exec("delete from $send_memo_table where no=? and member_no=?", [$del[$i], $member['no']]);
    }
...
// 메모삭제
if ($exec == "del") {
    $connect->exec("delete from $send_memo_table where no=? and member_no=?", [$no, $member['no']]);
    Response::redirect(Request::scriptName() . "?page=$page");
}

// 선택된 메모가 있을시 데이타 뽑아오기;;
if ($no) {
    $now_data = $connect->row("select * from $send_memo_table where no=? and member_no = ?", [$no, $member['no']]);
    $now_data += array('name' => '', 'user_id' => '');
}


// 읽지 않은 쪽지의 개수 구하기
$new_total = (int)$connect->value("select count(*) from $send_memo_table where readed='1' and member_no=?", [$member['no']]);

// 전체 쪽지의 개수
$total = (int)$connect->value("select count(*) from $send_memo_table  where member_no=?", [$member['no']]);
...
// 데이타 뽑아오는 부분...
$result = $connect->all("select a.no as no, a.subject as subject, a.reg_date as reg_date, a.readed as readed, b.name as name, b.user_id as user_id, a.member_to as member_to from $send_memo_table a ,$member_table b where a.member_no=? and a.member_to=b.no  order by a.no desc limit ?,?", [$member['no'], (int)$start_num, (int)$page_num]);
...
if ($page > $show_page_num) {
    ...
    $print_page = "<a href=" . Request::scriptName() . "?id=" . e($id) . "&page=" . e($prev_page) . "&select_arrange=" . e($select_arrange) . "&desc=" . e($desc) . "&category=" . e($category) . "&sn=" . e($sn) . "&ss=" . e($ss) . "&sc=" . e($sc) . "&keyword=" . e($keyword) . "&page_num=" . e($page_num) . ">[Prev]</a> ";
    $print_page .= "<a href=" . Request::scriptName() . "?id=" . e($id) . "&page=1&select_arrange=" . e($select_arrange) . "&desc=" . e($desc) . "&category=" . e($category) . "&sn=" . e($sn) . "&ss=" . e($ss) . "&sc=" . e($sc) . "&keyword=" . e($keyword) . "&page_num=" . e($page_num) . ">[1]</a>..";
}

while ($i + $start_page <= $total_page && $i <= $show_page_num) {
    $move_page = $i + $start_page;
    if ($page == $move_page) $print_page .= " <b>$move_page</b> ";
    else $print_page .= "<a href=" . Request::scriptName() . "?id=" . e($id) . "&page=" . e($move_page) . "&select_arrange=" . e($select_arrange) . "&desc=" . e($desc) . "&category=" . e($category) . "&sn=" . e($sn) . "&ss=" . e($ss) . "&sc=" . e($sc) . "&keyword=" . e($keyword) . "&page_num=" . e($page_num) . ">[" . e($move_page) . "]</a>";
    $i++;
}

if ($total_page > $move_page) {
    $next_page = $move_page + 1;
    $print_page .= "..<a href=" . Request::scriptName() . "?id=" . e($id) . "&page=" . e($total_page) . "&select_arrange=" . e($select_arrange) . "&desc=" . e($desc) . "&category=" . e($category) . "&sn=" . e($sn) . "&ss=" . e($ss) . "&sc=" . e($sc) . "&keyword=" . e($keyword) . "&page_num=" . e($page_num) . ">[" . e($total_page) . "]</a>";
    $print_page = " <a href=" . Request::scriptName() . "?id=" . e($id) . "&page=" . e($next_page) . "&select_arrange=" . e($select_arrange) . "&desc=" . e($desc) . "&category=" . e($category) . "&sn=" . e($sn) . "&ss=" . e($ss) . "&sc=" . e($sc) . "&keyword=" . e($keyword) . "&page_num=" . e($page_num) . ">[Next]</a>";
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
 ...
 <td nowrap>&nbsp;&nbsp;&nbsp;<a href=member_memo.php><img src=images/vi_B_inbox.gif border=0></a> <a href=member_memo2.php><img src=images/vi_B_sent.gif border=0></a> <a href=member_memo3.php><img src=images/vi_B_write.gif border=0></a></td>
    <td align=right><img src="images/t.gif" width="10" height="10"><font color="#666666">전체 :
      <b><?= e($total) ?></b> , 읽지 않은 쪽지 : <b><?= e($new_total) ?></b></font>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  </tr>
</table>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="3">
        <tr>
          ...
          <td><img src="images/t.gif" width="10" height="3"><br>
            <a href=javascript:void(window.open('view_info.php?member_no=<?= e($now_data['member_to']) ?>','view_info','width=400,height=510,toolbar=no,scrollbars=yes'))><?= $now_data['name']?></a> <font style=font-size:8pt;>(<b>ID</b> : <?= e($now_data['user_id']) ?>)</td>
        </tr>
        <tr>
    ...
    ?>
        ...
        <tr onMouseOver=this.style.backgroundColor="#FFF5F5" onMouseOut=this.style.backgroundColor="">
          ...
          <td width="80" align="center"><img src="images/t.gif" width="10" height="3"><br>
            <a href=javascript:void(window.open('view_info.php?member_no=<?= e($data['member_to']) ?>','view_info','width=400,height=510,toolbar=no,scrollbars=yes'))><?= e($data['name']) ?></a><br><font style=font-size:8pt;color:999999>(<?= e($data['user_id']) ?>)</td>
