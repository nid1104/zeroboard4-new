...
if ( (!empty($member['no']) && $member['is_admin'] < 3 && $member['is_admin'] >= 1)
 || (!empty($member['no']) && $member['board_name'])) Response::redirect('admin_setup.php');
else {
    if (!empty($member['no'])) {
...
<input type=hidden name=s_url value="<?= Request::uri() ?>">
