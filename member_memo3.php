...
if ($keyword) {
    ...
    if (!$status) { $s_que = " where user_id like ? or name like ? "; $s_params = array($kw_like, $kw_like); }
}

// 전체 회원의 수
$temp2 = $connect->value("select count(*) from $member_table  $s_que", $s_params);
$total_member = (int) $temp2;
...
// 오프라인 멤버
if (!$status) {
    $que = "select * from $member_table $s_que order by no desc limit ?,?";
    ...
    for ($i = $start_num; $i < $endnum; $i++) {
        $_in_marks[] = '?';
        $s_params[] = substr($_connector[$i], 12);
    }
    if ($_in_marks) $s_que = " where no in (" . implode(',', $_in_marks) . ") ";
    $que = "select * from $member_table $s_que";
    $result = $connect->all($que, $s_params);
}
...
if ($page > $show_page_num) {
    ...
    $print_page = "<a href=" . Request::scriptName() . "?page=" . e($prev_page) . "&status=" . e($status) . "&keyword=" . e($keyword) . ">[Prev]</a> ";
    $print_page .= "<a href=" . Request::scriptName() . "?page=1&status=" . e($status) . "&keyword=" . e($keyword) . ">[1]</a>..";
}

while ($i + $start_page <= $total_page && $i <= $show_page_num) {
    $move_page = $i + $start_page;
    if ($page == $move_page) $print_page .= " <b>$move_page</b> ";
    else $print_page .= "<a href=" . Request::scriptName() . "?page=" . e($move_page) . "&status=" . e($status) . "&keyword=" . e($keyword) . ">[" . e($move_page) . "]</a>";
    $i++;
}

if ($total_page > $move_page) {
    $next_page = $move_page + 1;
    $print_page .= "..<a href=" . Request::scriptName() . "?page=" . e($total_page) . "&status=" . e($status) . "&keyword=" . e($keyword) . ">[" . e($total_page) . "]</a>";
    $print_page .= " <a href=" . Request::scriptName() . "?page=" . e($next_page) . "&status=" . e($status) . "&keyword=" . e($keyword) . ">[Next]</a>";
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
 ...
 <td>&nbsp;&nbsp;&nbsp;<a href=member_memo.php><img src=images/vi_B_inbox.gif border=0></a> <a href=member_memo2.php><img src=images/vi_B_sent.gif border=0></a> <a href=member_memo3.php><img src=images/vi_B_write.gif border=0></a></td>
    <td align=right><font color="#666666">전체회원수 : <?= e($total) ?></font>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  </tr>
</table>
...
foreach ($result as $data) {
    ...
    $stat = "<img src=images/memo_offline.gif>";
   ...
   <tr align=center height=23 onMouseOver=this.style.backgroundColor=\"#FFF5F5\" onMouseOut=this.style.backgroundColor=\"\" style=cursor:hand; onclick=location.href=\"javascript:void(window.open('view_info.php?member_no=" . e_js($data['no']) . "','view_info','width=400,height=510,toolbar=no,scrollbars=yes'))\">
      <td style='word-break:break-all;'>$kind</td>
      <td style='word-break:break-all;'><img src=images/t.gif width=10 height=3><br><a href=javascript:void(window.open('view_info.php?member_no=" . e_js($data['no']) . "','view_info','width=400,height=510,toolbar=no,scrollbars=yes'))>" . e($user_id) . "</a></td>
...
<?php $checked[$status] = "checked"; ?>
  <td colspan=5 height=30>
     ...
     <tr>
     <td><input type=text name=keyword value="<?= e($keyword) ?>" size=20 style=font-size:9pt;width:100px;height:20px;></td>
     ...
     <td><a href="<?= Request::scriptName() ?>?page=<?= e($page) ?>"><img src=images/memo_cancel.gif border=0></a></td>
     </tr>
     </table>
