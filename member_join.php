...
$group_name = Request::req('group_name');
...
// 그룹 번호 체크
if (!$group_no) {
    $tmpResult = $connect->row("select * from $group_table order by no limit 1");
...
if ($id) {
    ...
    if ($group_name) $group = $connect->row("select * from $group_table where name=?", [$group_name]);
    elseif ($group_no) $group = $connect->row("select * from $group_table where no=?", [$group_no]);
    if (empty($group['no'])) Error("지정된 그룹이 존재하지 않습니다");
    if (!$group['use_join'] && !$mode) Error("현재 지정된 그룹은 추가 회원을 모집하지 않습니다");

}


if (!$referer) $referer = Request::header('Referer');
...
<input type=hidden name=group_no value="<?= e($group['no']) ?>">
...
<?php } ?>
  ...
  <tr align=right height=28>
     ...
     <td align=left>&nbsp;<input type=text name=email size=50 maxlength=255 value="<?= e($member['email']) ?>" style=border-color:#d8b3b3 class=input>
  ...
  <tr align=right height=28>
     ...
     <td align=left>&nbsp;<input type=text name=homepage size=50 maxlength=255 value="<?= e($member['homepage']) ?>" style=border-color:#d8b3b3 class=input>
...
<?php if ($group['use_aol']) { ?>
  <tr align=right height=28>
     ...
     <td align=left>&nbsp;<input type=text name=aol size=20 maxlength=20 value="<?= e($member['aol']) ?>" style=border-color:#d8b3b3 class=input>
...
<?php if ($group['use_hobby']) { ?>
  <tr align=right height=28>
     ...
     <td align=left>&nbsp;<input type=text name=hobby size=50 maxlength=50 value="<?= e($member['hobby']) ?>" style=border-color:#d8b3b3 class=input>
...
<?php if ($group['use_comment']) { ?>
  <tr align=right height=28>
     ...
     <td align=left>&nbsp;<textarea cols=50 rows=4 name=comment style=border-color:#d8b3b3 class=textarea><?= e($member['comment']) ?></textarea><br>&nbsp;<input type=checkbox value=1 name=open_comment checked> 공개</td>
