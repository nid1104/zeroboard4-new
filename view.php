 ...
 * 공통 파일 include
 **************************************************************************/
if (empty($_view_included)) {
    $_view_included = false;
...
$email = '';
$prev_email = $next_email = '';
$show_ip = '';
$prev_data = $next_data = array();
$check_ref = 0;
$view_result = array();
$number = '';
$a_prev = $a_next = '';
$prev_subject = $prev_name = $prev_hit = $prev_vote = $prev_reg_date = $prev_icon = $prev_face_image = $prev_comment_num = '';
$next_subject = $next_name = $next_hit = $next_vote = $next_reg_date = $next_icon = $next_face_image = $next_comment_num = '';
$hide_prev_start = $hide_prev_end = $hide_next_start = $hide_next_end = '';
$hide_sitelink1_start = $hide_sitelink1_end = $hide_sitelink2_start = $hide_sitelink2_end = '';
$hide_download1_start = $hide_download1_end = $hide_download2_start = $hide_download2_end = '';
$hide_homepage_start = $hide_homepage_end = $hide_email_start = $hide_email_end = '';
$hide_comment_start = $hide_comment_end = $hide_c_password_start = $hide_c_password_end = '';
...
// 사용권한 체크
if ($setup['grant_view'] < $member['level'] && !$is_admin) Error("사용권한이 없습니다", "login.php?id=$id&page=$page&page_num=$page_num&category=$q_category&sn=$q_sn&ss=$q_ss&sc=$q_sc&keyword=$q_keyword&no=$no&s_url=" . urlencode(Request::uri()));
...
if (!$setup['use_alllist']) {
    $_dbTimeStart = getmicrotime();
    if ($data['prev_no']) $prev_data = $connect->row("select * from {$t_board}_{$id} where no=?", [$data['prev_no']]);
    if ($data['next_no']) $next_data = $connect->row("select * from {$t_board}_{$id} where no=?", [$data['next_no']]);
    $_dbTime += getmicrotime() - $_dbTimeStart;
}
...
if (!$setup['use_alllist']) {
    $_dbTimeStart = getmicrotime();
    $check_ref = (int)$connect->value("select count(*) from {$t_board}_{$id} where division=? and headnum=?", [$data['division'], $data['headnum']]);
    if ($check_ref > 1) $view_result = $connect->all("select * from {$t_board}_{$id} where division=? and headnum=? order by headnum desc,arrangenum", [$data['division'], $data['headnum']]);
    $_dbTime += getmicrotime() - $_dbTimeStart;
}

// 간단한 답글의 데이타를 가지고옴;;
$_dbTimeStart = getmicrotime();
$view_comment_result = $connect->all("select * from {$t_comment}_{$id} where parent=? order by no asc", [$no]);
$_dbTime += getmicrotime() - $_dbTimeStart;
...
if ($data['is_secret'] && !$is_admin && $data['ismember'] != $member['no'] && $member['level'] > $setup['grant_view_secret']) {
    if ($member['no']) {
        $secret_check = $connect->value("select count(*) from {$t_board}_{$id} where headnum=? and ismember=?", [$data['headnum'], $member['no']]);
        if (!$secret_check) error("비밀글을 열람할 권한이 없습니다");
    } else {
        $secret_rows = $connect->all("select password from {$t_board}_{$id} where headnum=?", [$data['headnum']]);
        $secret_ok = false;
        foreach ($secret_rows as $secret_row) {
            if ($secret_row['password'] !== '' && $secret_row['password'] !== null && verifyHash($password, $secret_row['password'])) { $secret_ok = true; break; }
        }
        if (!$secret_ok) {
            head();
            ...
            Session::set('zb_s_check', $secret_str);
        }
    }
}

// 현재글의 HIT수를 올림;;
if (strpos(Session::get('zb_hit', ''), $setup['no'] . "_" . $no) === false) {
    $_dbTimeStart = getmicrotime();
    $connect->exec("update {$t_board}_{$id} set hit=hit+1 where no=?", [$no]);
    $_dbTime += getmicrotime() - $_dbTimeStart;
...
if ($data['prev_no'] && !$setup['use_alllist']) {
    ...
    $prev_subject = e($prev_data['subject'] = $prev_data['subject']) . " " . $prev_comment_num;
    $prev_name = e($prev_data['name'] = $prev_data['name']);
    ...
    $prev_hit = $prev_data['hit'];
    $prev_vote = $prev_data['vote'];
...
if ($data['next_no'] && !$setup['use_alllist']) {
    ...
    $next_subject = e($next_data['subject'] = $next_data['subject']) . " " . $next_comment_num;
    $next_name = e($next_data['name'] = $next_data['name']);
    ...
    $next_hit = $next_data['hit'];
    $next_vote = $next_data['vote'];
...
if ($data['homepage'] && zb_url_scheme_ok($data['homepage'])) $a_homepage = "<a onfocus=blur() href='" . e($data['homepage']) . "' target=_blank>"; else $a_homepage = "<Zetx";
...
// 글쓰기버튼
if ($is_admin || $member['level'] <= $setup['grant_write']) $a_write = "<a onfocus=blur() href='write.php?$href$sort&no=$no&mode=write&sn1=$q_sn1'>"; else $a_write = "<Zeroboard ";

// 답글 버튼
if (($is_admin || $member['level'] <= $setup['grant_reply']) && $no && $data['headnum'] > -2000000000) $a_reply = "<a onfocus=blur() href='write.php?$href$sort&no=$no&mode=reply&sn1=$q_sn1'>"; else $a_reply = "<Zeroboard ";

// 목록 버튼
if ($is_admin || $member['level'] <= $setup['grant_list']) $a_list = "<a onfocus=blur() href='zboard.php?id=$id&page=$page&page_num=$page_num&category=$q_category&sn=$q_sn&ss=$q_ss&sc=$q_sc&keyword=$q_keyword&prev_no=$no&sn1=$q_sn1&divpage=$divpage&select_arrange=$select_arrange&desc=$desc'>"; else $a_list = "<Zeroboard  ";
...
// 추천버튼
if (strpos(Session::get('zb_vote', ''), $setup['no'] . "_" . $no) === false) $a_vote = "<a onfocus=blur() href='vote.php?$href$sort&no=$no&zb_csrf_token=" . Session::csrfToken() . "'>";
...
if ($member['no']) {
    $c_name = e($member['name']); $hide_c_password_start = "<!--"; $hide_c_password_end = "-->";
...
} else $c_name = "<input type=text name=name size=8 maxlength=10 class=input value=\"" . e(Session::get('zb_writer_name', '')) . "\">";
...
$data['homepage'] = ($data['homepage'] && zb_url_scheme_ok($data['homepage'])) ? e($data['homepage']) : '';
...
// 코멘트 출력;;
if ($setup['use_comment']) {
    foreach ($view_comment_result as $c_data) {
        $c_data['ip'] = e($c_data['ip']);
        $comment_name = e($c_data['name']);
...
if ($check_ref > 1) {
    ...
    foreach ($view_result as $data) {
        ...
        if ($data['no'] == $no) $number = "<img src=\"" . e($dir) . "/arrow.gif\" border=0>"; else $number = "&nbsp;";
